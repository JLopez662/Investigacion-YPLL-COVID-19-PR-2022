                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    radioButtons("testType", 
                                 label = "Tipos de pruebas | Tipos de muertes (YPLL)",
                  ...
                  mainPanel(
                    ...
                    tabsetPanel(id = "tabs",
                                ...
                                tabPanel("Mapa",
                                         ...
                                         #numericInput("min_rate", "Minimo", value = 0, min = 0, max = 100, step = 1 ),
                                         #numericInput("max_rate", "Maximo", value = 10, min = 0, max = 100, step = 1),
                                         
                                         #plotOutput("mapa_positividad2"),
                                         #numericInput("min_rate2", "Minimo", value = 0, min = 0, max = 100, step = 1 ),
                                         #numericInput("max_rate2", "Maximo", value = 5, min = 0, max = 100, step = 1),
                                ),
...
server <- function(input, output, session) {
  ...
  by_region <- reactive({load(file.path(rda_path,"region.rda"));
    ...
    DT::datatable(ret, #class = 'white-space: nowrap',
                  ...
                  options = list(dom = 't', pageLength = -1,
                                 columnDefs = list(
                                   ...
                                   list(targets = 8, visible = FALSE), #8
  ...
  output$mapa_positividad <- renderPlot({
    ...
    if (input$testType == "Antigens" || input$testType == "Molecular")
    {
    ...
    plot_map(tests_by_strata,  
             ...
             #max_rate = input$max_rate/100
    )
    }
    else
    {
      load(file.path(rda_path,"ypll_deaths.rda"))
      
      plot_map2(ypll_deaths,  
                start_date = input$range[1], 
                end_date = input$range[2],
                type =  input$testType,
                #min_rate2 = input$min_rate2/100,
                #max_rate2 = input$max_rate2/100
      )
    }
  })
  ...
  datasetInput <- reactive({
    switch(input$dataset,
           ...
           "municipios" = {
             load(file.path(rda_path,"ypll_deaths.rda"))
             tests_by_strata %>%
               filter(testType %in% c("Male", "Female")) %>%
               mutate(patientCity = as.character(patientCity)) %>%
               filter(patientCity %in% c("No reportado", poblacion_municipios$patientCity)) %>%
               group_by(testType, patientCity, date) %>%
               summarize(positives = sum(positives), tests = sum(tests),
