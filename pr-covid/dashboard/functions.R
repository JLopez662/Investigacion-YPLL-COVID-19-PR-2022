...
plot_positivity <- function(tests, 
  ...
  the_ylim <- dat %>%
    ...
    summarize(lower = min(lower, 0), upper = max(upper, 100))
...
plot_map <- function(tests_by_strata,
  ...
  ret <<- tests_by_strata %>%
    ...
    scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Reds"),
                         ...
                         #limits= c(min_rate, max_rate)
                         limits= c(100*min_rate, 100*max_rate)
                         ) +
    coord_map() +
    theme_void() +
    theme(legend.position = "bottom") +
    ggtitle(paste("Por ciento de pruebas positivas por municipio para el periodo de",
...
plot_map2 <- function(ypll_deaths,
                     start_date = first_day, 
                     end_date = last_complete_day, 
                     type = "Female",
                     min_rate2 = 0.00,
  ...
  ret <<- ypll_deaths %>%
    ...
    summarize(positives  = sum(positives),
              ...
              rate       = positives / tests, .groups = "drop") %>%
    ungroup() %>%
    mutate(rate = 100*pmin(pmax(rate, min_rate2), max_rate2)) %>%
    na.omit() %>%
    rename(municipio = patientCity) %>%
    left_join(map, by = "municipio") %>%
    ggplot() + 
    ...
    scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Blues"),
                         name = "Por ciento de casos de muertes:",
                         #limits= c(min_rate, max_rate)
                         limits= c(100*min_rate2, 100*max_rate2)
    ) +
    coord_map() +
    theme_void() +
    theme(legend.position = "bottom") +
    ggtitle(paste("Por ciento de casos de muerte por municipio para el periodo de",
                  format(start_date, "%B %d"),
                  "a",
                  format(end_date, "%B %d."), "\n",
                  paste("Casos", 
                        case_when(type == "Female" ~ "de muertes de mujeres por COVID-19", 
                                  type == "Male" ~ "de muertes de hombres por COVID-19",
                                  type == "Molecular+Antigens" ~ "moleculares y de antígenos"))))
  return(ret)
}
